...
import { ClothingItem, Outfit, Season, Occasion } from '../types/wardrobe';
...
export const useOutfitRecommendation = (initialOptions?: Partial<OutfitGenerationOptions>) => {
  ...
  const generateRecommendations = useCallback(async (options?: Partial<OutfitGenerationOptions>) => {
    if (items.length < 2) {
      setState(prev => ({
        ...prev,
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to generate outfits';
  ...
  const selectOutfit = useCallback((index: number) => {
  ...
  const nextOutfit = useCallback(() => {
    setState(prev => ({
      ...prev,
      selectedOutfitIndex: (prev.selectedOutfitIndex + 1) % Math.max(1, prev.outfits.length),
    }));
  }, []);
  ...
  const previousOutfit = useCallback(() => {
    setState(prev => ({
      ...prev,
      selectedOutfitIndex: prev.selectedOutfitIndex > 0 
        ? prev.selectedOutfitIndex - 1 
        : Math.max(0, prev.outfits.length - 1),
    }));
  }, []);

  const saveCurrentOutfit = useCallback((name?: string) => {
    if (state.outfits.length === 0 || state.selectedOutfitIndex >= state.outfits.length) {
      return null;
    }
    
    const selectedOutfit = state.outfits[state.selectedOutfitIndex];
    const outfitName = name || `Generated Outfit ${new Date().toLocaleDateString()}`;
  ...
  const getWeatherBasedRecommendation = useCallback(async (weatherData: WeatherData) => {
  ...
  const getOccasionBasedRecommendation = useCallback(async (occasion: Occasion) => {
