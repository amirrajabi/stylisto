-- ================================
-- Complete Users Table Enhancement Script
-- Execute this in Supabase Dashboard SQL Editor
-- ================================

-- First, let's see the current structure
-- (You can run this separately to check current columns)
...
-- Add style preferences (JSON format for flexibility)
ALTER TABLE users
...
SET 
  ...
  color_preferences = COALESCE(color_preferences, '{}'),
  brand_preferences = COALESCE(brand_preferences, '{}'),
  budget_range = COALESCE(budget_range, '{"min": 0, "max": 1000, "currency": "AUD"}'),
  privacy_settings = COALESCE(privacy_settings, '{"profile_visibility": "private", "data_sharing": false}'),
  notification_settings = COALESCE(notification_settings, '{"push_notifications": true, "email_notifications": true, "outfit_recommendations": true, "weather_alerts": true}'),
...
WHERE 
  ...
  style_preferences IS NULL OR 
  color_preferences IS NULL OR 
  brand_preferences IS NULL OR 
  budget_range IS NULL OR 
