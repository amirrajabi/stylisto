...
export const validateOutfit = (items: ClothingItem[]): { isValid: boolean; errors: string[] } => {
  const errors: string[] = [];
  ...
  const hasTop = categories.includes(ClothingCategory.TOPS) || categories.includes(ClothingCategory.DRESSES);
  const hasBottom = categories.includes(ClothingCategory.BOTTOMS) || categories.includes(ClothingCategory.DRESSES);
...
export const suggestOutfitItems = (selectedItems: ClothingItem[], allItems: ClothingItem[]): ClothingItem[] => {
  ...
  const suggestions = allItems.filter(item => {
    ...
    const hasMatchingSeason = item.season.some(season => selectedSeasons.includes(season));
    const hasMatchingOccasion = item.occasion.some(occasion => selectedOccasions.includes(occasion));
  ...
  const complementaryCategories: Record<ClothingCategory, ClothingCategory[]> = {
    [ClothingCategory.TOPS]: [ClothingCategory.BOTTOMS, ClothingCategory.SHOES, ClothingCategory.ACCESSORIES],
    [ClothingCategory.BOTTOMS]: [ClothingCategory.TOPS, ClothingCategory.SHOES, ClothingCategory.ACCESSORIES],
    [ClothingCategory.DRESSES]: [ClothingCategory.SHOES, ClothingCategory.ACCESSORIES, ClothingCategory.OUTERWEAR],
    [ClothingCategory.SHOES]: [ClothingCategory.TOPS, ClothingCategory.BOTTOMS, ClothingCategory.DRESSES],
    [ClothingCategory.OUTERWEAR]: [ClothingCategory.TOPS, ClothingCategory.BOTTOMS, ClothingCategory.DRESSES],
    [ClothingCategory.ACCESSORIES]: [ClothingCategory.TOPS, ClothingCategory.BOTTOMS, ClothingCategory.DRESSES],
  ...
  const neededCategories = selectedCategories.flatMap(cat => complementaryCategories[cat]);
...
export const getWardrobeInsights = (items: ClothingItem[]): string[] => {
  ...
  const mostWornCategory = Object.entries(categoryCount).reduce((a, b) => 
    categoryCount[a[0] as ClothingCategory] > categoryCount[b[0] as ClothingCategory] ? a : b
  )[0];
  ...
  if (unwornItems.length > 0) {
    insights.push(`You have ${unwornItems.length} items that haven't been worn yet`);
  }
  ...
  insights.push(`Consider adding more ${leastRepresentedSeason} items to your wardrobe`);
