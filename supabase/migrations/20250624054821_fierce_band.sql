/*
  ...
  1. New Types
    - `feedback_type` enum for categorizing different AI feedback types
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE polname = 'Users can insert own ai feedback'
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE polname = 'Users can select own ai feedback'
    AND tablename = 'ai_feedback'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE polname = 'Users can update own ai feedback'
    AND tablename = 'ai_feedback'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE polname = 'Users can delete own ai feedback'
    AND tablename = 'ai_feedback'
  ) THEN
