/*
  # AI Feedback Table Creation

  1. New Types
    - `feedback_type` enum for categorizing different types of AI feedback

  2. New Tables
    - `ai_feedback` - Stores user feedback on AI-generated content
      - `id` (uuid, primary key)
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE TRIGGER update_ai_feedback_updated_at
...
-- Create RLS policies (using IF NOT EXISTS to avoid errors)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can insert own ai feedback'
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can select own ai feedback'
    AND tablename = 'ai_feedback'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can update own ai feedback'
    AND tablename = 'ai_feedback'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can delete own ai feedback'
    AND tablename = 'ai_feedback'
  ) THEN
