...
class VisionAIService {
  ...
  private readonly CATEGORY_MAPPING: Record<string, { category: ClothingCategory; confidence: number }> = {
    // Tops
    ...
    'running shorts': { category: ClothingCategory.ACTIVEWEAR, confidence: 0.8 },
  ...
  async analyzeClothing(imageUri: string): Promise<ClothingAnalysisResult> {
    try {
      ...
      const visionResponse = data.responses[0] as VisionResponse;
   ...
   */
  private processVisionResponse(response: VisionResponse): ClothingAnalysisResult {
    ...
    if (response.labelAnnotations) {
      ...
      // Determine category
      let bestCategoryMatch = { category: ClothingCategory.TOPS, confidence: 0 };
      
      for (const { label, score } of labels) {
        const mapping = this.CATEGORY_MAPPING[label];
        if (mapping && score >= mapping.confidence && score > bestCategoryMatch.confidence) {
      ...
      for (const { label, score } of labels) {
        ...
        if (seasons && score > 0.6) {
          seasons.forEach(season => {
            seasonMatches.set(season, Math.max(seasonMatches.get(season) || 0, score));
          });
        }
      }
      ...
      result.confidence.seasons = result.seasons.length > 0
        ? Array.from(seasonMatches.values()).reduce((sum, val) => sum + val, 0) / result.seasons.length
        : 0;
      ...
      for (const { label, score } of labels) {
        ...
        if (occasions && score > 0.6) {
          occasions.forEach(occasion => {
            occasionMatches.set(occasion, Math.max(occasionMatches.get(occasion) || 0, score));
          });
        }
      }
      ...
      result.confidence.occasions = result.occasions.length > 0
        ? Array.from(occasionMatches.values()).reduce((sum, val) => sum + val, 0) / result.occasions.length
        : 0;
    }
