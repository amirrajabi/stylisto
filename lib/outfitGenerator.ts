import { ClothingItem, Outfit, ClothingCategory, Season, Occasion } from '../types/wardrobe';
...
class OutfitGenerator {
  ...
  generateOutfits(
    ...
    if (mergedOptions.season) {
      availableItems = availableItems.filter(
        item => item.season.includes(mergedOptions.season as Season)
      );
    }
    ...
    if (mergedOptions.occasion) {
      availableItems = availableItems.filter(
        item => item.occasion.includes(mergedOptions.occasion as Occasion)
      );
    }
    
    // Filter by weather if specified
    if (mergedOptions.weather) {
      availableItems = this.filterItemsByWeather(availableItems, mergedOptions.weather);
    }
  ...
  private generateOutfitCombinations(
    ...
    const forcedItems = items.filter(item => forceIncludeItems.includes(item.id));
    ...
    const buildOutfit = (
      ...
      for (const item of categoryItems) {
        ...
        if (!this.isItemCompatible(item, currentOutfit)) continue;
   ...
   */
  private isItemCompatible(item: ClothingItem, outfit: ClothingItem[]): boolean {
    // Check if outfit already has this category (except for accessories)
    if (item.category !== ClothingCategory.ACCESSORIES &&
        outfit.some(existingItem => existingItem.category === item.category)) {
      return false;
    }
    
    // Check if item has at least one season in common with the outfit
    if (outfit.length > 0) {
      const outfitSeasons = new Set(outfit.flatMap(item => item.season));
      if (outfitSeasons.size > 0 && !item.season.some(season => outfitSeasons.has(season))) {
        return false;
      }
    }
    
    // Check if item has at least one occasion in common with the outfit
    if (outfit.length > 0) {
      const outfitOccasions = new Set(outfit.flatMap(item => item.occasion));
      if (outfitOccasions.size > 0 && !item.occasion.some(occasion => outfitOccasions.has(occasion))) {
        return false;
      }
    }
   ...
   */
  private calculateItemCompatibilityScore(item: ClothingItem, outfit: ClothingItem[]): number {
    ...
    const outfitSeasons = new Set(outfit.flatMap(item => item.season));
    const seasonOverlap = item.season.filter(season => outfitSeasons.has(season)).length;
    const seasonScore = outfitSeasons.size > 0 ? seasonOverlap / outfitSeasons.size : 1;
    totalScore += seasonScore * 0.3;
    
    // Occasion overlap score
    const outfitOccasions = new Set(outfit.flatMap(item => item.occasion));
    const occasionOverlap = item.occasion.filter(occasion => outfitOccasions.has(occasion)).length;
    const occasionScore = outfitOccasions.size > 0 ? occasionOverlap / outfitOccasions.size : 1;
  ...
  private scoreOutfit(
    ...
    const scoreBreakdown = {
      ...
      styleMatching: this.calculateStyleMatchingScore(items, options.stylePreference),
      occasionSuitability: this.calculateOccasionSuitabilityScore(items, options.occasion),
      seasonSuitability: this.calculateSeasonSuitabilityScore(items, options.season),
      weatherSuitability: options.weather ? 
        this.calculateWeatherSuitabilityScore(items, options.weather) : 1,
      userPreference: this.calculateUserPreferenceScore(items, options.preferredColors),
      variety: this.calculateVarietyScore(items),
    };
    
    // Calculate weighted total score
    const totalScore = Object.entries(scoreBreakdown).reduce(
      (total, [key, score]) => total + score * SCORE_WEIGHTS[key as keyof typeof SCORE_WEIGHTS],
      0
    );
    ...
    if (this.recentlyGeneratedOutfits.has(outfitKey)) {
      const daysSinceLastGenerated = (Date.now() - this.recentlyGeneratedOutfits.get(outfitKey)!.getTime()) / 
        (24 * 60 * 60 * 1000);
      
      // Apply penalty that decreases over time
      const recencyPenalty = Math.max(0, 0.5 * (1 - daysSinceLastGenerated / 7));
      return {
   ...
   */
  private determineColorHarmony(hslColors: { h: number; s: number; l: number }[]): string {
   ...
   */
  private calculateColorDistanceScore(hslColors: { h: number; s: number; l: number }[]): number {
    ...
    for (let i = 0; i < hslColors.length; i++) {
      for (let j = i + 1; j < hslColors.length; j++) {
        totalDistance += this.calculateColorDistance(hslColors[i], hslColors[j]);
        pairs++;
      }
    }
  ...
  private calculateStyleMatchingScore(
    ...
    const categoryStyleValues: Record<ClothingCategory, Partial<StylePreference>> = {
    ...
    const itemStyleValues = items.map(item => {
      ...
      for (const tag of item.tags) {
        ...
        if (conservativeTags.some(conservativeTag => tag.toLowerCase().includes(conservativeTag))) {
          boldness -= 0.1;
        }
      }
    ...
    const outfitStyle = {
      formality: itemStyleValues.reduce((sum, item) => sum + item.formality, 0) / itemStyleValues.length,
      boldness: itemStyleValues.reduce((sum, item) => sum + item.boldness, 0) / itemStyleValues.length,
    };
    
    // Calculate distance from preferred style
    const formalityDiff = Math.abs(outfitStyle.formality - stylePreference.formality);
    const boldnessDiff = Math.abs(outfitStyle.boldness - stylePreference.boldness);
  ...
  private calculateWeatherSuitabilityScore(
    ...
    const hasOuterwear = items.some(item => item.category === ClothingCategory.OUTERWEAR);
    ...
    if (weather.precipitation > 0.5 || weather.conditions === 'rainy' || weather.conditions === 'snowy') {
  ...
  private calculateVarietyScore(items: ClothingItem[]): number {
    ...
    for (const [key, date] of this.recentlyGeneratedOutfits.entries()) {
      ...
      if (similarity > 0.8) {
        const daysSinceGenerated = (Date.now() - date.getTime()) / (24 * 60 * 60 * 1000);
  ...
  private calculateOutfitSimilarity(key1: string, key2: string): number {
    ...
    return commonItems.length / (items1.length + items2.length - commonItems.length);
  }

  /**
   * Filter items based on weather conditions
   */
  private filterItemsByWeather(items: ClothingItem[], weather: WeatherData): ClothingItem[] {
  ...
  private groupItemsByCategory(items: ClothingItem[]): Record<ClothingCategory, ClothingItem[]> {
    const result: Record<ClothingCategory, ClothingItem[]> = {} as Record<ClothingCategory, ClothingItem[]>;
  ...
  private ensureOutfitVariety(outfits: GeneratedOutfit[]): GeneratedOutfit[] {
    ...
    for (let i = 1; i < outfits.length; i++) {
      ...
      for (const selectedKey of selectedKeys) {
        const similarity = this.calculateOutfitSimilarity(currentKey, selectedKey);
        if (similarity > 0.7) {
          isUnique = false;
          break;
        }
  ...
  createOutfit(
    ...
    const tags = Array.from(new Set(
      items.flatMap(item => item.tags).slice(0, 5)
    ));
